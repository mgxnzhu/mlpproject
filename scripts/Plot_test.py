...
for modelname, colorname in zip(models, colors):
    ...
    ax.plot(range(num_steps), stats[:,0], label="loss(train)", color=colorname, alpha=0.5, linestyle='--')
    ax.plot(range(num_steps), stats[:,3], label="loss(test)", color=colorname)
...
for modelname, colorname in zip(models, colors):
    ...
    ax.plot(range(num_steps), stats[:,4], label="acc(test)", color=colorname)
lgd = ax.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
               ncol=2, mode="expand", borderaxespad=0)
ax.set_xlim(0, num_steps)
ax.set_xlabel("step")
...
for modelname, colorname in zip(models, colors):
    stats = np.load(STAT_DIR+modelname+".npy")
    ax.plot(range(num_steps), stats[:,2], label="f1(train)", color=colorname, alpha=0.5, linestyle='--')
    ax.plot(range(num_steps), stats[:,5], label="f1(test)", color=colorname)
lgd = ax.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
               ncol=2, mode="expand", borderaxespad=0)
ax.set_xlim(0, num_steps)
ax.set_xlabel("step")
